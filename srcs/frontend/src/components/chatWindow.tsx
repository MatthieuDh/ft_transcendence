    ...
    <Flex direction="column" h="100%">
      ...
      <VStack flex={1} p={4} overflowY="auto" gap={3} bg="gray.50" _dark={{ bg: "gray.900" }} alignItems="flex-start">
        {messages.length === 0 ? (
          ...
          messages.map((msg, index) => {
            ...
            const timestamp = (msg as unknown as Record<string, unknown>).Time as string || msg.createdAt || new Date().toISOString();
      ...
      <Flex p={3} bg="white" _dark={{ bg: "gray.800", borderColor: "gray.700" }} borderBottomRadius="lg" borderTop="1px solid" borderColor="gray.200">
        <Input
          ...
          disabled={isSending}
        />
        <Button size="sm" colorPalette="purple" borderRadius="full" onClick={handleSend} loading={isSending}>
          Send
        </Button>
      </Flex>
    </Flex>
