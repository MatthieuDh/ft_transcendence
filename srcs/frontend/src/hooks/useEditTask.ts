...
export function useEditTask(task: Task | null, onTaskUpdate: () => void) {
  ...
  const cancelEdit = (field: 'title' | 'description' | 'deadline') => {
    if (!task) return;
    if (field === 'title') { setIsEditingTitle(false); setEditTitle(task.title); }
    if (field === 'description') { setIsEditingDesc(false); setEditDesc(task.description || ''); }
    if (field === 'deadline') { setIsEditingDeadline(false); setEditDeadline(task.deadline ? new Date(task.deadline).toISOString().split('T')[0] : ''); }
  };
